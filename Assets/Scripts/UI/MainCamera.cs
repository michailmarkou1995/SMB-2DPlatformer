...
namespace UI
{
    public class MainCamera : MonoBehaviour
    {
        ...
        private void Start()
        {
            PlayerController mario = FindObjectOfType<PlayerController>();
            ...
            if (Camera.main != null) cameraWidth = Camera.main.orthographicSize * aspectRatio;
            ...
            if (rightEdge.position.x - leftEdge.position.x <= cameraWidth * 2)
            {
                ...
                transform.position = new Vector3((leftEdge.position.x + rightEdge.position.x) / 2f, targetPosition.y,
                    targetPosition.z);
                canMove = false;
            }
                    ...
                    targetPosition.x - leftEdge.position.x >= cameraWidth - followAhead)
                {
                    if (canMoveBackward || target.transform.position.x + followAhead >= transform.position.x)
                    {
                        targetPosition = new Vector3(targetPosition.x + followAhead, targetPosition.y,
                            targetPosition.z);
                        transform.position =
                            Vector3.Lerp(transform.position, targetPosition, smoothing * Time.deltaTime);
                    }
                }
