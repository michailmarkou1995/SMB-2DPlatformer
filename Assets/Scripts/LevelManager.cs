...
public class LevelManager : MonoBehaviour {
	...
	void Start () {
		...
		mario_Animator = playerController.gameObject.GetComponent<Animator> ();
		mario_Rigidbody2D = playerController.gameObject.GetComponent<Rigidbody2D> ();
		playerController.UpdateSize ();
	...
	public void MarioInvincibleStarman() {
		...
		AddScore (starmanBonus, playerController.transform.position);
	}

	IEnumerator MarioInvincibleStarmanCo() {
		isInvincibleStarman = true;
		mario_Animator.SetBool ("isInvincibleStarman", true);
		playerController.gameObject.layer = LayerMask.NameToLayer ("Mario After Starman");
		if (hurryUp) {
		...
		playerController.gameObject.layer = LayerMask.NameToLayer ("Mario");
		if (hurryUp) {
			ChangeMusic (levelMusicHurry);
		} else {
			ChangeMusic (levelMusic);
	...
	IEnumerator MarioInvinciblePowerdownCo() {
		...
		playerController.gameObject.layer = LayerMask.NameToLayer ("Mario After Powerdown");
	...
	public void MarioPowerUp() {
		...
		AddScore (powerupBonus, playerController.transform.position);
	}
	...
	public void MarioCompleteCastle() {
		...
		playerController.AutomaticWalk(playerController.CastleWalkSpeedX);
	}
	...
	public void MarioReachFlagPole() {
		...
		playerController.ClimbFlagPole ();
	}
}
